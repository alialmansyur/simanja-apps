...
const MainCalendar = ({ events = [], selectedEvent, setSelectedEvent, kpiData }) => {
  ...
  const extendedKpiData = useMemo(() => ({
    ...kpiData,
    todayAgendas: todayEventsCount
  }), [kpiData, todayEventsCount]);

  return (
    <div className="flex-1 flex flex-col min-h-[600px] relative xl:px-2">
      <div className="flex flex-col xl:flex-row xl:justify-between xl:items-center mb-6 gap-4">
        <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Kalender Kegiatan</h3>
        <div className="flex items-center space-x-3 w-full xl:w-auto justify-start xl:justify-end">
          <button 
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm bg-white dark:bg-slate-800 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 w-full sm:w-64 text-slate-700 dark:text-slate-200 transition-colors"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl overflow-hidden p-5 calendar-container">
                          ...
                          {selectedEvent.participants?.length > 0 ? (
                            selectedEvent.participants.map((p, idx) => (
                              <tr key={idx} className="bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-4 py-2.5 font-mono text-[11px] text-slate-600 dark:text-slate-400">{p.nip || '-'}</td>
                                <td className="px-4 py-2.5 text-slate-600 dark:text-slate-400">{p.name || (typeof p === 'string' ? p : '-')}</td>
                              </tr>
                            ))
                          ) : (
